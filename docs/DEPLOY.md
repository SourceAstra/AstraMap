...
| 语言 | 最终语义 Provider | 实时语法层 |
|------|------------------|------------|
| Go | `scip-go` | 内置 Tree-sitter |
| TypeScript / JavaScript | `scip-typescript` | 内置 Tree-sitter |
| Python | `scip-python` | 内置 Tree-sitter |
| Java / Kotlin / Scala | `scip-java` | 内置 Tree-sitter |
| C / C++ | `scip-clang` | 内置 Tree-sitter |
| Rust | `rust-analyzer` | 内置 Tree-sitter |
| C# | `scip-dotnet` | 内置 Tree-sitter |
| Ruby | `scip-ruby` | 内置 Tree-sitter |

PHP、Dart、Visual Basic、Bash、Swift、Lua、Zig 不属于 Supported Registry。
...
```bash
# 发布构建机执行；客户机器不需要 Go、CGO 或 grammar 构建工具
./build.sh
...
Core 在发布构建阶段静态链接 12 种正式语言的 Tree-sitter grammar，因此构建机需要 CGO/C 工具链；
客户机器仍只接收单个二进制，并按需安装 SCIP Provider。
...
构建 Core 不下载或编译任何 grammar 源码。Go Module 自身仍按标准 Go 构建规则解析普通依赖。
...
1. 检测并让用户选择 Supported Registry 中的语言；
2. 检查所选语言的 SCIP Provider、项目清单与构建前置条件；
3. Tree-sitter 立即建立实时文件结构；SCIP 依赖缺失时明确保留 `semanticDirty`；
4. 生成或导入 SCIP protobuf，校验实际输出覆盖全部所选语言；
5. 提交 SCIP 语义基线；
6. 按当前源码重建内置 Tree-sitter 实时层并清除已收敛文件的 `semanticDirty`。
...
其他 Provider 的安装提示由 Registry 按所选语言输出。grammar 在 AstraMap 发布构建阶段编入，
用户机器不会现场编译 Syntax Worker。

### 3.2 索引标志

| 标志 | 说明 |
|------|------|
| `--lang c,python` | 明确选择语言 |
| `--scip index.scip` | 导入已有 SCIP 文件；实际语言必须与选择一致 |
| `--scip-only` | 只导入 SCIP，显式跳过本次 Tree-sitter 实时层重建 |
| `--refresh-scip` | 强制重新生成并导入 SCIP |
| `--full` | 刷新 SCIP 后强制重建 Tree-sitter 实时层 |
| `--watch [秒]` | 监听变化；Tree-sitter 实时提交后尝试刷新 SCIP |

`--treesitter-only`、`--scip-file` 和“无 SCIP 自动回退”均已删除。
...
|------|----------|--------------|
| Go | `scip-go` | `go.mod` |
| TypeScript / JavaScript | `scip-typescript` | Node.js 与项目依赖；TS 通常需要 `tsconfig.json` |
| Python | `scip-python` | Python 环境与项目依赖 |
| Java / Kotlin / Scala | `scip-java` | Gradle、Maven 或 sbt 等受支持构建入口 |
| C / C++ | `scip-clang` | 有效 `compile_commands.json`；必要时可用 `bear -- make` 生成 |
| Rust | `rust-analyzer` | `Cargo.toml` |
| C# | `scip-dotnet` | `.sln` 或 `.csproj` |
| Ruby | `scip-ruby` | `Gemfile` 等 Ruby 项目清单 |

Provider 缺失、前置条件不足、命令失败、产物损坏或输出语言不匹配都会阻断本次索引。

### 3.4 SCIP 与 Syntax Overlay

- `scip` 是最终跨文件语义基线；
- `syntax-package` 是内置 Tree-sitter 产生的当前文件实时事实，也覆盖尚未进入编译单元的新文件；
- 源文件变化时先提交实时语法层，并立即失效涉及该文件的旧 SCIP 关系；
- 每次 SCIP 全量替换会先失效旧语法层，再基于当前源码重建；
- `heuristic` 仅表示数据库级补充解析，不构成语言准入能力。

### 3.5 持续更新

`amap watch` 检测到变化后先用内置 Tree-sitter 原子替换变化文件，使新增、修改、重命名和删除
立即可见；随后按 ProjectUnit 刷新 SCIP。Provider 失败不会撤销实时层，而会保留
`semanticDirtyCount`、`semanticDirtyFiles` 和错误诊断，禁止返回涉及脏文件的旧 SCIP 关系。
...
| Codex / Antigravity | MCP + `AGENTS.md` 规则 | `codex mcp add` | 写入 `~/.codex/config.toml` | `mcp_servers` 键 |
...
下一步：构建代码地图索引
  amap index                    # 检查所选语言依赖并建立 SCIP 语义基线
  amap index --refresh-scip     # 强制重新生成并导入 SCIP
  amap index --scip index.scip  # 导入与所选语言匹配的现有 SCIP
```
...
**Codex / Antigravity**（`~/.codex/config.toml`）：
...
或手动编辑 `~/.codex/config.toml`（Antigravity 与 Codex 共享此环境）：
...
Codex / Antigravity 要求为每个 MCP 工具显式设置 `approval_mode = "approve"`，否则每次调用都会弹出确认提示。
...
```bash
# 默认从 0.0.0.0:3000 开始监听，端口冲突时自动 +1，后台运行并打印本机/内网访问地址、PID、日志路径
  ...
  quality-gate:
    ...
    steps:
      ...
      - name: Build Index
        ...
        # 缺少所选语言的 SCIP Provider 或项目依赖时直接失败
        # CI 建议显式使用: amap index --refresh-scip --project .
...
crontab -e
# 添加：
0 * * * * /usr/local/bin/amap index --refresh-scip --project /path/to/project >> /var/log/astramap-index.log 2>&1
```
...
RUN go mod download
COPY . .
RUN CGO_ENABLED=0 go build -o /amap ./cmd/amap
...
| Syntax Overlay | `<lang>:<rel_path>::<qualified_name>` | `ruby:lib/service.rb::Service.query` |
...
核心服务:
  ...
  amap index [--project <path>] [--lang <ids>] [--scip <path>] [--scip-only] [--refresh-scip] [--full]  构建/更新索引
  amap watch [seconds] [--project <path>]           变化后完整刷新 SCIP 并重建可选 Overlay
